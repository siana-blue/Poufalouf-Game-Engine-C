...
PfAnimation::PfAnimation(unsigned int textureIndex, const PfRectangle& textCoordRectangle, bool loop, unsigned int soundIndex, const PfColor& color) : m_loop(loop), m_over(false), m_currentFrame(1)
{
    addFrame(new PfAnimationFrame(textureIndex, textCoordRectangle, soundIndex, color));
}

PfAnimation::PfAnimation(DataPackage& data) : m_loop(data.nextChar()==1), m_over(false), m_currentFrame(data.nextUInt())
{
	for (unsigned int i=0, size=data.nextUInt();i<size;i++)
...
const PfAnimationFrame& PfAnimation::currentFrame() const
{
    ...
    if (m_currentFrame > mp_frames_v.size())
        throw PfException(__LINE__, __FILE__,
                          string("La frame actuelle est ") +
                          itostr(m_currentFrame) + ". Le nombre de frames " +
                          "total est " + itostr(mp_frames_v.size()) + ".");
...
PfAnimationGroup::PfAnimationGroup() : m_currentAnimation(NO_ANIMATION) {}
...
void PfAnimationGroup::addAnimation(PfAnimation* p_anim, PfAnimationStatus value)
{
    if (p_anim == 0)
        throw ArgumentException(__LINE__, __FILE__,
                                "l'animation à ajouter est nulle.",
                                "p_anim", "PfAnimationGroup::addAnimation");
    if (value == NO_ANIMATION)
        throw ArgumentException(__LINE__, __FILE__,
                                string("la valeur de l'animation à ajouter") +
                                " est NO_ANIMATION.",
                                "value", "PfAnimationGroup::addAnimation");
...
{
    if (m_currentAnimation == NO_ANIMATION)
        throw PfException(__LINE__, __FILE__, "L'animation actuelle est NO_ANIMATION.");
...
const PfAnimationFrame& PfAnimationGroup::currentFrame() const
{
    ...
    try
    {
        if (m_currentAnimation == NO_ANIMATION)
            throw PfException(__LINE__, __FILE__,
                              "L'animation actuelle est NO_ANIMATION.");
        map<PfAnimationStatus, PfAnimation*>::const_iterator it = mp_animations_map.find(m_currentAnimation);
        if (it == mp_animations_map.end() || it->second == 0)
            throw PfException(__LINE__, __FILE__,
                              "L'animation en cours est introuvable.");
        p_frame = &(it->second->currentFrame());
    }
    catch (PfException& e)
    {
        throw PfException(__LINE__, __FILE__,
                          "Impossible de récupérer la frame en cours", e);
    }
